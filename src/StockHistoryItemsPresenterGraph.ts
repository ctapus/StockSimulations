...
export default class StockHistoryItemsPresenterGraph {
    ...
    public drawHistoricDataGraph(): void {
    ...
    public drawLegend() {
        ...
        const dict = [["Day open", this.openDayColor],
        ["50 simple moving average", this.sma50DaysColor], ["100 simple moving average", this.sma100DaysColor], ["200 simple moving average", this.sma200DaysColor],
        ["50 exponential moving average", this.ema50DaysColor], ["100 exponential moving average", this.ema100DaysColor], ["200 exponential moving average", this.ema200DaysColor]];
        for(var i: number=0; i<dict.length; i++) {
            g.append("rect").attr("x", legendOffsetX).attr("y", legendOffsetY + i * (squareLength + 10)).attr("width", squareLength).attr("height", squareLength).style("fill", dict[i][1]);
            g.append("text").attr("x", legendOffsetX + squareLength + 10).attr("y", legendOffsetY + i * (squareLength + 10) + 10).style("fill", "black").text(dict[i][0]).attr("text-anchor", "left").style("alignment-baseline", "middle");
        }
    }
}